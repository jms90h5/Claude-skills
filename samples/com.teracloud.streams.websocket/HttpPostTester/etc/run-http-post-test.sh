...
# different Streams instance. That will test the HttpPost operator's 
...
streamtool submitjob -d d1 -i i2 ~/workspace32/HttpPostTester/output/com.teracloud.streams.websocket.sample.HttpPostTester.sab -P Url=http://b0513:8080/MyServices/Banking/Deposit -P NumSenders=1 -P createPersistentHttpConnection=true -P LogHttpPostActions=true -P MaxMessageRate=50.0 -P MessageBatchingTime=6.0 -P httpTimeout=30 -P delayBetweenConsecutiveHttpPosts=0 -P tlsAcceptAllCertificates=true -P maxRetryAttempts=5 -P waitTimeBetweenRetry=5000 -P httpStatusCodesThatRequireRetry="503, 408, 504"
