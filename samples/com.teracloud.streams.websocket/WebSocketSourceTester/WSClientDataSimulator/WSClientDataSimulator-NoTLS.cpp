...
This C++ example below can be used to generate data traffic to test the
 toolkit example named WebSocketSourceTester.
(i.e. test it using non-tls endpoint i.e. plain WebSocket with no security)
This C++ application is useful to test the Streams application mentioned above in the
absence of a full fledged remote client test infrastructure. This C++ application
can become handy to test the  toolkit's WebSocketSource operator.
So, this application will mimic the way a real client application will
interact with the Streams application that is ready to ingest data from remote WebSocket client.
...
with the Streams application that runs the WebSocketSource (server side) operator.
...
---------
[
This client is mainly used to test the WebSocketSource from the  toolkit or
...
1) Ensure that you have installed boost_1_69_0 as explained in the  toolkit documentation.

2) Ensure that you have installed websocket++ v0.8.1 or higher as explained in the  documentation.
...
application running in Streams. You can use the WAV files available from the audio-files directory in the samples
...
then the Streams application can be connected to the real client application traffic to perform the intended operation.
...
the remote WebSocket server application if that Streams application is configured to send back
...
int main(int argc, char *argv[]) {
    // We are going to make a single WebSocket connection to the Streams application that
	// invokes the  WebSocketSource operator.
    if (argc < 7) {
